...
| 4 | Basic OCR for simple text/numbers | Done, pending Windows smoke test | Codex (`codex/v0.3-ocr`) | `vision/ocr.py` adds an `OcrEngine` protocol, optional `PytesseractEngine`, ROI measurement, digit whitelisting, and explicit confidence thresholding. `agent/ocr_state_bridge.py` writes accepted text to `GameState`; fake-engine tests cover the pure logic. Claude will install real Tesseract and live-smoke-test after the PR lands. |
