...
| 7 | Live Windows smoke test | Done (2026-09-24) | Claude | Windows 11, 3840x2160 at 150% scale, Notepad as the target, 10 fps. Session A: 1:07, 672 frames, **0 dropped**, 1373 events; enabling input control stopped it (`stop: input_control_enabled`) and greyed out Record (clicking it did nothing). Session B: 0:09, 94 frames, 0 dropped; F8 stopped it at once (`stop: f8`, marker written, `session.json` complete). Switching windows emitted `focus` lost/gained (7 events), and no input was written while focus was lost. Keys and clicks sent by the computer-use tool (SendInput, injected) were **all dropped**. 20 real `mouse_move` events from the user's own mouse over Notepad were recorded in client pixels inside 942x1179. Real keyboard input was not exercised live; unit tests cover it. `list`, `validate --all` (both OK), `export` (672 rows, 27 actions aligned to `[t, next_t)`) and `review` (overlay, ←/→, `q` quits, exit 0) all worked on the real sessions. Also checked: **DPI**: importing `dxcam` (through `core.capture`) makes the process per-monitor DPI aware (awareness 2), so pynput hook coordinates and `client_region()` are both physical pixels and agree. This is implicit, a follow-up could set it explicitly. **hwnd**: `list_visible_windows` uses `EnumWindows`, which returns top-level windows only, so the `GetForegroundWindow() == hwnd` check is sound. `safety-reviewer` ran on tasks 3/4/5/6 (PASS WITH NOTES, all fixed). Nothing recorded was committed. |
| R | Release close-out (v0.5.0) | Done (2026-09-24) | Claude | `CHANGELOG.md` v0.5.0 entry, `docs/ROADMAP.md` (v0.5 → Completed, next TBD), `docs/ARCHITECTURE.md` (recording layer), `README.md` (v0.5 overview, `scripts/recordings.py` usage, injected input not recorded), `APP_VERSION = "0.5.0"`, script banners. Feature→`main` via PR #43 as a **merge commit**; closes Issue #41. |
