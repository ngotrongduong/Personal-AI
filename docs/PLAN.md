...
|---|------|--------|-------|-------|
| 0 | Kickoff | In progress | Claude | Issue #82, `feature/v1.0-personal-agent` from `main`, this file, `AGENTS.md` / `docs/HANDOFF.md` / `docs/ROADMAP.md`, draft PR feature→`main`. |
| 1 | `agent/skill_effects.py` + profile `expect` | Todo | Claude (Codex out of quota until 2026-09-25 13:55) | Pure tests: parse/validate, defaults, unknown field and undeclared detector rejected, confirmed/not_seen for both `visible` values, observations before the finish ignored, save round-trip, import-boundary test. |
| 2 | `agent/agent_session.py` + planner `max_run_minutes` / `stop_when` + `check_model` | Todo | Claude | Pure tests: each preflight check, required vs advisory, budget cap and expiry, goal freshness and "after start", loader validation, `check_model` with a fake transport. |
