...
| 1 | `agent/skills.py` + `ActionIntent`/`VisibilityRule` extensions | Todo | Codex (Claude while Codex is out of quota) | Pure tests: forbidden keys, allowlist, hold cap, click/press/hold → intent. |
| 2 | `agent/profile.py` load/save/list + `profiles/example/profile.json` | Todo | Codex | Temp-dir tests: path traversal, bad references, missing template, save→load round trip, no overwrite. |
