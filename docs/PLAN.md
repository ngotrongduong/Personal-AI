...
| 1 | `agent/session_log.py` | Done | Claude (Codex out of quota until 2026-09-25 13:55) | `tests/test_session_log.py`, 13 tests. Pure tests: every record type, unknown type/field rejected, text cleaned and cut, 5 MB cap writes one `truncated`, I/O error turns the writer off without raising, `close` writes one `session_end`, reader rejects bad order/fields. |
